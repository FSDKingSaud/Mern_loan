...
const TopupLoan = () => {
  ...
  const { allLoans, status } = useSelector(
    (state) => state.loanReducer
  );

  const [myTopUpLoan, setMyTopUpLoan] = useState(null)
  ...
  useEffect(() => {
  if(allLoans){
    setMyTopUpLoan(allLoans.filter(loan => loan.isTopUpLoan))
  }
  }, [allLoans]);
  ...
  return (
    <div className="MLoan">
      ...
      >
        ...
        <tbody>
          {!myTopUpLoan || status === "loading" ? (
            <tr>
              <td colSpan="8">
                <PageLoader width="70px" />
              </td>
            </tr>
          ) : myTopUpLoan && myTopUpLoan.length === 0 ? (
            <tr>
              <td colSpan="8" style={{ textAlign: "center" }}>
                No Top-Up loan record
              </td>
            </tr>
          ) : (
            myTopUpLoan.map((loan) => (
